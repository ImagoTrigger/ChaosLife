...
while {true} do
{
	...
	while {!(vehicle player == player) && driver vehicle player == player} do
	{
		...
		if (!isNull _cargo && BTC_Hud_Cond) then
		{
			...
			if ((abs _cargo_z) > BTC_lift_max_h) then {_arrow ctrlSetText "\images\arrow_down_ca.paa";};
			if ((abs _cargo_z) < BTC_lift_min_h) then {_arrow ctrlSetText "images\arrow_up_ca.paa";};
			if ((abs _cargo_z) > BTC_lift_min_h && (abs _cargo_z) < BTC_lift_max_h) then {_arrow ctrlSetText "images\objective_complete_ca.paa";};
			if (!_can_lift && BTC_Hud_Cond) then {_arrow ctrlSetText "images\objective_incomplete_ca.paa";};
