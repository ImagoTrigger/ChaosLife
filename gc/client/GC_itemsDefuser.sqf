...
_DefuseBombArray = ["TimeBomb", "Mine", "MineE"];
if (isNil "INV_BombDefuserInUse") then {INV_BombDefuserInUse = false;};
...
if (not(INV_BombDefuserInUse)) then 
	...
	if (vehicle player == player) then 
		...
		{_bombs = _bombs + ( (position player) nearObjects  [_x, 3] );} forEach _DefuseBombArray;
		...
		for [{_i=0}, {_i < (count INV_ServerBombArray)}, {_i=_i+1}] do 
			...
			if (((INV_ServerBombArray select _i) select 1) == (vehicle player)) exitWith 
				...
				[((INV_ServerBombArray select _i) select 2), 1] call INV_AddInventoreItem;
				player groupChat localize "STRS_inv_items_defuser_success";
