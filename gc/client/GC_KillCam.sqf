...
diag_log format["KillCam: %1",_this];
...
if (_start) then {
	...
	0 fadesound 0.5; //MUF-commented
	0 fademusic 0.5; //MUF-commented
...
if (alive player) exitwith {"LOL" call bis_fnc_log; cuttext ["","plain"];}; //--- Terminate when player manages to switch do different unit already

if (isMultiplayer) then {disableUserInput true;};
...
if (_start) then {
	...
	_musicEH = addMusicEventHandler ["MusicStop",{[] spawn {playmusic format ['RadioAmbient%1',ceil random 30];};}];
