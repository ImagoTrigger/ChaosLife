private ["_transfer","_transfermoney","_steuern","_targetString","_withdraw","_art","_cash"];
_art  = _this select 2;
...
if (_art == "deposit") then {
  ...
  if (INV_PLAYERLIST select _transfer == player) exitWith {
    if (_cash < _transfermoney) then {
      ["GC_MoneyBad",["FIRST BANK OF STRATIS",format[localize "STRS_bank_deposit_self_no", (_transfermoney call ISSE_str_IntToStr), (bank_steuer call ISSE_str_IntToStr), "%"]]]  spawn bis_fnc_showNotification;
    } else {
      ["GC_Money",["FIRST BANK OF STRATIS",format[localize "STRS_bank_deposit_self_yes", (_transfermoney call ISSE_str_IntToStr)]]] spawn bis_fnc_showNotification;
...
if (_art == "withdraw") then {
  ...
  if (Kontostand >= _withdraw) then {
    ...
    ["GC_Money",["FIRST BANK OF STRATIS",format[localize "STRS_bank_withdraw_yes", (_withdraw call ISSE_str_IntToStr)]]]  spawn bis_fnc_showNotification;
  } else {
    ["GC_MoneyBad",["FIRST BANK OF STRATIS",localize "STRS_bank_no_money"]]  spawn bis_fnc_showNotification;
  };
};
