...
if (_art == "civmenu") then {
  if (!(createDialog "RscMenuCiv")) exitWith {hint "Dialog Error!";};

  civmenu_civ = _array select 1;
  civmenuciv  = _array select 2;
  sliderSetRange [3, 1, 15];
  sliderSetSpeed [3, 1, 1];
...
if (_art == "civinteraktion") then {
  if (!(createDialog "RscMenuCivInteraction")) exitWith {hint "Dialog Error!";};
