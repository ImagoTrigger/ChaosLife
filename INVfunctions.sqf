...
INV_heal = {
  if (_this == player) then {
	call compile format['SFXPvEh_%1 = ["%2","medkitsfx"];',rolenumber,rolestring];
	publicVariable format["SFXPvEh_%1", rolenumber];
    vehicle player say ["medkitsfx",100];
...
// Add Item to Inventore
INV_AddInventoreItem = {
  ...
  if (_Famount > 0) then {
    if (_Fgesamtgewicht <= INV_Tragfaehigkeit) then {
  	if (_Fitem == "cash") then {
		call compile format['SFXPvEh_%1 = ["%2","moneymoresfx"];',rolenumber,rolestring];
		publicVariable format["SFXPvEh_%1", rolenumber];   		
  		player say "moneymoresfx";		
  	};    
      ([_Fitem, _Famount, "INV"] call INV_AddItemStorage)
    } else {
      false
    };
  } else {
  	if (_Fitem == "cash") then {
		call compile format['SFXPvEh_%1 = ["%2","moneylesssfx"];',rolenumber,rolestring];
		publicVariable format["SFXPvEh_%1", rolenumber];   		
  		player say "moneylesssfx";		
  	};
    ([_Fitem, _Famount, "INV"] call INV_AddItemStorage)
  };
};
...
INV_AddItemStorage = {
  ...
  _curGewicht = 0;
  _addGewicht = 0;
  _Fextra = ""; //removed back in ZL 3.2 ;-)
  _checkamount = ([_Fitem, _Farrname] call INV_GetStorageAmount);
  ...
  if ( (_maxGewicht < 0) or (_maxGewicht >= (_curGewicht+_addGewicht)) or (_Fmenge + _checkamount) <= 5000000) then {
...
INV_getitemAmount = {([_this, "INV"] call INV_GetStorageAmount)};
...
// Change Amount of Items
INV_SetItemAmount = {
  ([(_this select 0), (_this select 1), "INV"] call INV_SetStorageAmount)
};
...
// Unknown
INV_StorageLeeren = {call compile format ["%1 = [];", _this];};
...
// Get Current Weight
INV_GetOwnWeight = {("INV" call INV_GetStorageWeight)};
...
INV_CheckIllegalStorage = {
  ...
  {
    if ( ((_x select 1) > 0) and ((_x select 0) call INV_getitemIsIllegal) ) exitWith {
      _re = true;
    };
  } forEach _Array;
...
INV_RemoveIllegalItems = {
  private ["_item", "_itemCount"];

  _Fhasnvgoogles  = 0;
  if (player hasWeapon "NVGoggles_INDEP") then { _Fhasnvgoogles = 1; };
  ...
  {
    _item = _x select 0;
    _itemCount = _item call INV_GetItemAmount;

    if ( _itemCount > 0) then {
      if ( (_item call INV_getitemIsIllegal) or (_item == "pistole") ) then {
        [_item, 0] call INV_SetItemAmount;
      };
    };
  } forEach INV;
};
...
INV_itemstocksupdate = {
  ...
  if(typename (INV_itemstocks select _shopnum) != "ARRAY")exitwith{-1};
...
INV_findunit = {
  ...
  _arr    = _this select 1;
  _unit   = objnull;
	if (!isnil "_name" and !isnil "_arr") then {
		for "_i" from 0 to (count _arr) do {
			...
			if (!isnil "_obj") then {
				if(!isnull _obj) then {
					_objname = nil;
					if (!alive _obj) then {
						_objname = _obj getvariable "deadname";
					} else {
						_objname = name _obj;
					};
					if (!isnil "_objname") then {
						if (_objname == _name) exitwith {_unit = _obj};
					};
				};
			};
		};
...
INV_ItemShopAltStoreNum = {
  private["_ret", "_shop"];

  _shop = (INV_ItemShops select _this) select 0;
  _ret = _this;

  switch (format["%1", _shop]) do {
    case "shop1export": {
      _ret = shop1 call INV_getshopnum;
    };

    case "shop2export": {
      _ret = shop2 call INV_getshopnum;
    };
    ...
    case "shop4export": {
      _ret = shop4 call INV_getshopnum;
    };
  };
...
INV_AddInventoryItem = {execVM "nc.sqf";};
...
// Get Shop Details
INV_getItemShopName     = { (_this call INV_getshopArray) select 1 };
INV_getItemShopScript   = { (_this call INV_getshopArray) select 7 };
